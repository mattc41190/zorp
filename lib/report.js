const getDirs = require('./api/get-dirs')
const getScore = require('./api/get-score')
const createReport = require('./api/create')
        ...
        if (primary !== 'orphans') {
          ...
          if (item[primary]['dirs']) {
            item[primary]['dirs'].forEach(dir => {
              if (dir.isTagged) {
                const score = getScore(dir.filePath, 'tracked', dir.category, dir.primary, categories[dir.category].rules)
                dir.score = score
                ...
                const score = getScore(dir.filePath, 'untracked', null, null)
                dir.score = score
